...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    if (extendedSession) {
      ...
      if (token && user) {
        ...
        // We might want to use the user info from the token/backend instead of session.user
        // But for now session.user should be synced via the jwt callback if we set it there
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await backendClient.post('/api/register', {
        name,
        email,
        phoneNumber,
        password,
